...
// ============================================================
// SMARTFLOW RENDERER v4.2 - Motor Isométrico 2.5D
// Integración completa con SmartFlowCatalog v4.1
// Correcciones: Nozzle tags para todos los puertos,
// Selección correcta para todos los tipos de equipo
// ============================================================
...
const SmartFlowRenderer = (function() {
    ...
    const ISO_CONFIG = {
        ...
        COLORS: {
            ...
            'PE': '#1e293b', 'PV': '#7c3aed', 'CP': '#fb923c',             'FR': '#8b5cf6', 'PT': '#a78bfa', 'RB': '#ec4899',
    ...
    const AnnotationManager = {
        ...
        getDirectionalOffsets(dir3D, w, h) {
            switch (dir3D) {
                case 'X': return [{ dx: 0, dy: -h - 8 }, { dx: 0, dy: h + 8 }, { dx: w + 15, dy: -h/2 }, { dx: -w - 15, dy: -h/2 }];
                case 'Z': return [{ dx: w + 15, dy: -h/2 }, { dx: -w - 15, dy: -h/2 }, { dx: 0, dy: -h - 8 }, { dx: 0, dy: h + 8 }];
                default: return [{ dx: 0, dy: -h - 8 }, { dx: 0, dy: h + 8 }, { dx: w + 15, dy: 0 }, { dx: -w - 15, dy: 0 }];
            }
        },
    ...
    function getComponentLabel(compType) {
        if (_catalog) { const comp = _catalog.getComponent(compType); if (comp && comp.abbr) return comp.abbr; }
        const fallback = {
    ...
    function checkOffsetCollision(p1, p2, offset) {
        const midPoint = { x: (p1.x + p2.x) / 2 + offset.dx, y: (p1.y + p2.y) / 2 + offset.dy, z: (p1.z + p2.z) / 2 + offset.dz };
    ...
    function drawOrigin() {
        ...
        _ctx.beginPath(); _ctx.moveTo(o.x - 25, o.y); _ctx.lineTo(o.x + 25, o.y);
    ...
    function drawPuertos(obj) {
        ...
        obj.puertos.forEach(port => {
            ...
            if (port.id && _cam.scale > 0.3) drawNozzleTag(proj, port, obj);
        });
    }
    ...
    // ═══════════════════════════════════════════════════════
    // CORREGIDO: Selección para TODOS los tipos de tanque
    // ═══════════════════════════════════════════════════════
    const CYLINDER_TYPES = ['tanque_v', 'torre', 'reactor', 'desgasificador', 'desmineralizador',
        'suavizador', 'filtro_carbon', 'filtro_arena', 'clarificador', 'columna_fraccionadora',
        'evaporador', 'cristalizador', 'absorbedor', 'stripper', 'reactor_encamisado',
        'autoclave', 'agitador', 'centrifuga_discos', 'tanque_aseptico', 'espesador',
        'separador', 'antorcha'];
    ...
    function drawSelection(element) {
        ...
        if (element.type === 'equipment') {
            ...
            if (CYLINDER_TYPES.includes(eq.tipo || '')) {
    ...
    function render() {
        ...
        _renderQueueCache.forEach(item => {
            if (item.type === 'EQUIPMENT') {
                ...
                if (CYLINDER_TYPES.includes(tipo)) drawTank(eq);
    ...
    // ================================================================
    // CONTROL DE CÁMARA - autoCenter HÍBRIDO
    // ================================================================
    function autoCenter(options = {}) {
        ...
        lines.forEach(line => {
            ...
            pts.forEach(p => {
                ...
                else { const dist = Math.hypot(p.x - centroid.x, p.y - centroid.y, p.z - centroid.z); if (dist < 15000) points.push(p); }
            });
        });
        ...
        points.forEach(p => { const rx = (p.x - p.z) * COS30; const ry = (p.x + p.z) * SIN30 - p.y; if (rx < minX) minX = rx; if (rx > maxX) maxX = rx; if (ry < minY) minY = ry; if (ry > maxY) maxY = ry; });
        ...
        _cam.scale = sc;
        _cam.panX = _canvas.width / 2 - ((minX + maxX) / 2) * sc;         _cam.panY = _canvas.height / 2 - ((minY + maxY) / 2) * sc;
    ...
    function zoom(delta, mouseX, mouseY) {
        ...
        if (mouseX !== undefined && mouseY !== undefined && clampedScale !== _cam.scale) { _cam.panX = mouseX - (mouseX - _cam.panX) * (clampedScale / _cam.scale); _cam.panY = mouseY - (mouseY - _cam.panY) * (clampedScale / _cam.scale); }
    ...
    function init(canvasElement, coreInstance, catalogInstance, notifyFn) {
        ...
        autoCenter(isMobile ? { padding: 20, minScale: 0.06, maxScale: 0.5 } : {});
        scheduleRender();
        console.log('✔ SmartFlowRenderer v4.2 - Nozzle tags + Selección corregida');
    }
