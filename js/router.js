...
const SmartFlowRouter = (function() {
    ...
    function getFittingLength(componentType, diameter) {
        ...
        try {
            ...
            if (!dims) return 0;
            const diamKey = diameter + '"' || String(diameter);
    ...
    function getPortPosition(obj, portId) {
        ...
        if (obj.pos && obj.pos.x !== undefined) {
            const puerto = obj.puertos ? obj.puertos.find(function(p) { return p.id === portId; }) : null;
            if (!puerto) return null;
            return { x: obj.pos.x + (puerto.relX || (puerto.relPos ? puerto.relPos.x : 0) || 0), y: obj.pos.y + (puerto.relY || (puerto.relPos ? puerto.relPos.y : 0) || 0), z: obj.pos.z + (puerto.relZ || (puerto.relPos ? puerto.relPos.z : 0) || 0) };
        }
        const pts = _core ? _core.getLinePoints(obj) : (obj._cachedPoints || obj.points3D || obj.points);
        if (!pts || pts.length === 0) return null;
        if (obj.puertos) {
            const puerto = obj.puertos.find(function(p) { return p.id === portId; });
    ...
    function findElbowForLine(material, diameter, angleDeg) {
        ...
        const allTypes = catalog.listComponentTypes();
        const elbowTypes = allTypes.filter(function(t) { return t.toUpperCase().includes('ELBOW'); });
        ...
        if (!bestMatch) {
            ...
            bestMatch = elbowTypes.find(function(t) {
    ...
    function ensureFittings(lineObj, fromObj, fromPortId, toObj, toPortId, diameter, material) {
        ...
        if (fromObj && fromPortId && puntos.length >= 2) {
            ...
            if (Math.abs(dotInicio) < 0.96) { 
                ...
                if (!lineObj.components.some(function(c) { return c.tag === codoInicial.tag; })) {
            ...
            if (fromObj && fromObj._cachedPoints) {
                ...
                if (!isNaN(fromDiam) && !isNaN(toDiam) && Math.abs(fromDiam - toDiam) > 0.1) {
                    ...
                    if (reducerType) {
                        ...
                        if (!lineObj.components.some(function(c) { return c.tag === reducer.tag; })) {
        ...
        for (let i = 1; i < puntos.length - 1; i++) {
            const pAnt = puntos[i - 1], pAct = puntos[i], pSig = puntos[i + 1];
            ...
            if (angleDeg > 15) {
                ...
                if (!lineObj.components.some(function(c) { return c.tag === codo.tag; })) {
    ...
    function procesarInterseccionesDeLinea(nuevaLinea) {
        ...
        if (!_core) return;
        const db = _core.getDb();
        const lineasExistentes = db.lines.filter(function(l) { return l.tag !== nuevaLinea.tag; });
                ...
                for (let j = 0; j < ptsExistente.length - 1; j++) {
                    ...
                    if (proj.distance < tolerancia) {
                        ...
                        if (puertoId) {
                            ...
                            if (lineaExistente.puertos) { const puerto = lineaExistente.puertos.find(function(p) { return p.id === puertoId; }); if (puerto) puerto.connectedLine = nuevaLinea.tag; }
    ...
    function routeBetweenPorts(fromEquipTag, fromPortId, toEquipTag, toPortId, diameter, material, spec) {
        ...
        if (!_core) { notifyUser('Core no inicializado', true); return null; }
        const db = _core.getDb();
        const fromObj = _core.findObjectByTag(fromEquipTag) || db.equipos.find(function(e) { return e.tag === fromEquipTag; }) || db.lines.find(function(l) { return l.tag === fromEquipTag; });
        let toObj = _core.findObjectByTag(toEquipTag) || db.equipos.find(function(e) { return e.tag === toEquipTag; }) || db.lines.find(function(l) { return l.tag === toEquipTag; });
        ...
        if (_core.getLinePoints(toObj) || toObj._cachedPoints || toObj.points3D || toObj.points) {
            ...
            if (!toPortId || toPortId === '') {
                ...
                if (esExtremo && !diffDiam) { nuevoPuertoId = toPortId; endPos = puntoConexion; }
                    ...
                    if (puertoInsertado) { nuevoPuertoId = puertoInsertado; toObj = _core.findObjectByTag(toEquipTag) || db.lines.find(function(l) { return l.tag === toEquipTag; }); }
        ...
        let uniqueWaypoints = waypoints.filter(function(pt, i, arr) { return i === 0 || distance(pt, arr[i-1]) > 1; });
        ...
        if (fromObj.puertos) { const pFrom = fromObj.puertos.find(function(p) { return p.id === fromPortId; }); if (pFrom) pFrom.connectedLine = tag; }
        if (toObj.puertos) { const pTo = toObj.puertos.find(function(p) { return p.id === nuevoPuertoId; }); if (pTo) pTo.connectedLine = tag; }
